...
make_A_supp_plot <- function(results_df, A_value, Sigma_value) {
  ...
  plot <- results_df %>% filter(A_val == A_value, Sigma_val == Sigma_value, !(est_method %in% c("vi_nopen", "mom_nopen")), as.numeric(iter) <= 100) %>%
...
#VISULALIZE FOR MOM SIMS
#function to make plots
make_A_mom_supp_plot <- function(results_df, A_value, Sigma_value, J_val) {
  
  plot <- results_df %>% filter(A_val == A_value, Sigma_val == Sigma_value, J == J_val, !(est_method %in% c("vi_nopen", "mom_nopen"))) %>%
    ggplot(mapping = aes(x = as.factor(n), y = value, color = metric)) +
    geom_boxplot() +
    labs(title = paste0("TPR and FPR for Penalized MoM, A: ", A_value, ", Sigma: ", Sigma_value),
         x = "n") +
    facet_wrap(~est_method) +
    theme_bw()
...
#get A support results
mom_A_supp_results <- readRDS("mom_bic2_results_combined/mom_sims_A_support_results.RDS")

#get description of sim settings and join to above 
load("settings_df.Rdata")
...
A_val_vec <- unique(mom_A_supp_results$A_val)
Sigma_val_vec <- unique(mom_A_supp_results$Sigma_val)
J_val_vec <- unique(mom_A_supp_results$J)

mom_A_support_plots_list <- mapply(function (x,y,z) {make_A_mom_supp_plot(mom_A_supp_results, A_value = x, Sigma_value = y, J_val = z)}, 
                               rep(rep(A_val_vec, length(Sigma_val_vec)),2), 
                               rep(rep(Sigma_val_vec, each = length(A_val_vec)),2),
                               rep(J_val_vec, each = length(A_val_vec)*length(Sigma_val_vec)),
                               SIMPLIFY = FALSE)

mom_small_A_plots <- mom_A_support_plots_list[which(sapply(mom_A_support_plots_list, function (x) {x$A_val == 0.2}))]
mom_med_A_plots <- mom_A_support_plots_list[which(sapply(mom_A_support_plots_list, function (x) {x$A_val == 0.5}))]
mom_large_A_plots <- mom_A_support_plots_list[which(sapply(mom_A_support_plots_list, function (x) {x$A_val == 0.8}))]

mom_med_J_plots <- mom_A_support_plots_list[which(sapply(mom_A_support_plots_list, function (x) {x$J == 25}))]
mom_large_J_plots <- mom_A_support_plots_list[which(sapply(mom_A_support_plots_list, function (x) {x$J == 50}))]

#small A value plots
(mom_small_A_plots[[1]]$plot + mom_small_A_plots[[4]]$plot) / 
(mom_small_A_plots[[2]]$plot + mom_small_A_plots[[5]]$plot) / 
(mom_small_A_plots[[3]]$plot + mom_small_A_plots[[6]]$plot)

#medium A value plots
(mom_med_A_plots[[1]]$plot + mom_med_A_plots[[4]]$plot) / 
(mom_med_A_plots[[2]]$plot + mom_med_A_plots[[5]]$plot) / 
(mom_med_A_plots[[3]]$plot + mom_med_A_plots[[6]]$plot) 

#large A value plots
(mom_large_A_plots[[1]]$plot + mom_large_A_plots[[4]]$plot) / 
(mom_large_A_plots[[2]]$plot + mom_large_A_plots[[5]]$plot) / 
(mom_large_A_plots[[3]]$plot + mom_large_A_plots[[6]]$plot) 


# medium J plots
mom_small_A_plots <- which(sapply(mom_med_J_plots, function (x) {x$A_val == 0.2}))
mom_med_A_plots <- which(sapply(mom_med_J_plots, function (x) {x$A_val == 0.5}))
mom_large_A_plots <- which(sapply(mom_med_J_plots, function (x) {x$A_val == 0.8}))

(mom_med_J_plots[[mom_small_A_plots[1]]]$plot + mom_med_J_plots[[mom_small_A_plots[2]]]$plot + mom_med_J_plots[[mom_small_A_plots[3]]]$plot) /
(mom_med_J_plots[[mom_med_A_plots[1]]]$plot + mom_med_J_plots[[mom_med_A_plots[2]]]$plot + mom_med_J_plots[[mom_med_A_plots[3]]]$plot) /
(mom_med_J_plots[[mom_large_A_plots[1]]]$plot + mom_med_J_plots[[mom_large_A_plots[2]]]$plot + mom_med_J_plots[[mom_large_A_plots[3]]]$plot) 

#medium J plot
ggmatrix(lapply(mom_med_J_plots, function (x) {x$plot}), 3, 3,
         xAxisLabels = paste0("A: ", A_val_vec),
         yAxisLabels = paste0("Sigma: ", Sigma_val_vec),
         title = "TPR and FPR for Penalized MoM, J: 25",
         xlab = "n",
         ylab = "value")

#large J plot
ggmatrix(lapply(mom_large_J_plots, function (x) {x$plot}), 3, 3,
         xAxisLabels = paste0("A: ", A_val_vec),
         yAxisLabels = paste0("Sigma: ", Sigma_val_vec),
...
mom_A_supp_results %>%
  ...
  group_by(est_method, n, J, A_val, Sigma_val) %>%
