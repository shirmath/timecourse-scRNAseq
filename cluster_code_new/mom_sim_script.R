...
iteration <- ifelse(is.na(task_num), 2, (task_num - 1) %% 10 + 1)
...
sim_setting_idx <- ifelse(is.na(task_num), 1, (task_num-1) %/% 10 + 1)
...
lambda_file <- paste0("Setting_",sprintf("%02d", sim_setting_idx),"/sim_lambda_", sprintf("%02d", iteration), ".RDS")
saveRDS(sim_lambda_results, file = lambda_file)

#Store lambda results
full_A_file <- paste0("Setting_",sprintf("%02d", sim_setting_idx),"/sim_full_A_", sprintf("%02d", iteration), ".RDS")
